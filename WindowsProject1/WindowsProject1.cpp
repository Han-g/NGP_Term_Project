...
DWORD WINAPI ClientMain(LPVOID arg)
{
    ...
    retval = recv(sock, (char*)&clientNum, sizeof(int), 0); // 0,1,2,3
    InitObjects(clientNum);
    ...
    while (1) {
        ...
        if (nready == SOCKET_ERROR) {
            ...
            if (FD_ISSET(sock, &wset)) {
                if (Server_bufArray[clientNum].buf.object_info.capacity() != 0) {
                    Serialize(&Server_bufArray[clientNum].buf, buffer, BUFSIZE);
                ...
                if (retval == SOCKET_ERROR || retval == 0) {
                    ...
                    for (const char& c : buffer) {
                        if (c != 0) {
                            ...
                            if (Server_bufArray[clientNum].buf.object_info.size() != 0
                                && buf.buf.object_info.capacity() != 0) {
                                Server_bufArray[clientNum].buf.object_info.clear();
                                size_t objSize = Server_bufArray[clientNum].buf.object_info.capacity() * sizeof(obj_info);
                                Server_bufArray[clientNum].buf.object_info.reserve(objSize);
                                Server_bufArray[clientNum].buf.object_info = buf.buf.object_info;
                            }
                            break;
                        }
                    }
